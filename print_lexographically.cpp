// lexographical order is like dictionary order here we have to print like 

/*
	A = [1,2,3]
	[],[1],[1,2],[1,2,3],
	[2],[2,3],
	[3]

	in dictionary order
*/

#include <bits/stdc++.h>

using namespace std;

#define max(a,b) (a<b? b:a)
#define min(a,b) (a<b? a:b)
#define mod 1e9+7
#define FOR(a,c) for(int (a)=0;(a)<(c);(a)++)
#define FORL(a,b,c) for(int (a)=(b);(a)<=(c);(a)++)
#define FORR(a,b,c) for(int (a)=(b);(a)>=(c);(a)--)
#define INF 1000000000000000003
typedef long long int ll;
typedef vector<int> vi;
typedef pair<int,int> pi;
#define F first
#define S second
#define PB push_back
#define POB pop_back
#define MP make_pair
template <class T>
T modadd(T a,T b,ll m){return (a%b+b%m)%m;}
template <class T>
T modsub(T a,T b,ll m){T ans = (a%m)-(b%m);if(ans<0) ans+=m;return ans;}
template <class T>
T modmul(T a,T b,ll m){ return ((a%m)*(b%m))%m; }
vector<int> inputArray(int n){vector<int> arr(n);int i=0; FOR(i,n) cin>>arr[i];return arr;}

void lexographical(vector<int> arr,int index,vector<int> &result,vector<vector<int>> &ans){
	ans.push_back(result);
	cout<<"[";
	for(auto x:result){
		cout<<x<<" ";
	}
	cout<<"]"<<endl;
	if(index==arr.size()) return;
	
	for(int i=index;i<arr.size();i++){
		result.push_back(arr[i]);
		lexographical(arr,i+1,result,ans);
		result.pop_back();
	}
	return ;
}

int main(){
	int n ;cin>>n;
	auto arr = inputArray(n);

	vector<vector<int> > ans;
	vector<int> res;
	lexographical(arr,0,res,ans);
	cout<<endl;
	return 0;
}