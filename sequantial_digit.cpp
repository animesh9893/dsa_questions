// https://leetcode.com/problems/sequential-digits/
#include <bits/stdc++.h>

using namespace std;

#define max(a,b) (a<b? b:a)
#define min(a,b) (a<b? a:b)
#define mod 1e9+7
#define FOR(a,c) for(int (a)=0;(a)<(c);(a)++)
#define FORL(a,b,c) for(int (a)=(b);(a)<=(c);(a)++)
#define FORR(a,b,c) for(int (a)=(b);(a)>=(c);(a)--)
#define INF 1000000000000000003
typedef long long int ll;
typedef vector<int> vi;
typedef pair<int,int> pi;
#define F first
#define S second
#define PB push_back
#define POB pop_back
#define MP make_pair
template <class T>
T modadd(T a,T b,ll m){return (a%b+b%m)%m;}
template <class T>
T modsub(T a,T b,ll m){T ans = (a%m)-(b%m);if(ans<0) ans+=m;return ans;}
template <class T>
T modmul(T a,T b,ll m){ return ((a%m)*(b%m))%m; }
vector<int> inputArray(int n){vector<int> arr(n);int i=0; FOR(i,n) cin>>arr[i];return arr;}

void func(int digit,const int low,const int high,int element,vector<int> &ans){
	if(digit>=low and digit<=high){
		ans.push_back(digit);
	}
	if(element>9) return;
	if(element<=0) return;
	if(digit>high) return;
	digit = digit*10+element;
	func(digit,low,high,element+1,ans);
	return;
}

vector<int> sequentialDigits(int low, int high) {
    vector<int> ans;
    for(int i=1;i<9;i++){
    	func(i,low,high,i+1,ans);
    }
    // for(auto x:ans)
    	// cout<<x<<" ";
    // cout<<endl;
 	sort(ans.begin(),ans.end());
    return ans;
}

int main(){
	int low,high;cin>>low>>high;
	sequentialDigits(low,high);
	cout<<endl;
	return 0;
}